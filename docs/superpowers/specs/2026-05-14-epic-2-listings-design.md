...
The project uses Hibernate's `ddl-auto=update` (no Flyway). The table below is the **logical schema** that JPA generates from the `Category` entity; no hand-written DDL file is committed. Seed data is inserted by a Spring `ApplicationRunner` at startup that does an idempotent `INSERT … WHERE NOT EXISTS` per row.

```sql
-- Logical schema (generated by Hibernate from the Category entity)
...
Seed data (8 rows) inserted at startup if `categories` is empty:
```
...
Like `categories` (§3.1), the schema below is the **logical** version generated by Hibernate from the `Listing` entity — no hand-written DDL file. Indexes are declared via `@Index` annotations on `@Table`.
